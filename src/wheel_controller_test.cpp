...
#include <vector>
#include <atomic>
#include "wheel_controller_test.hpp"


Wheel_Controller_test::WheelControllerTest(const std::string& can_interface)
...
void  Wheel_Controller_test::update() {
    try {
...
void  Wheel_Controller_test::printPayload(const std::array<uint8_t, 8>& payload) {
    std::cout << "  Payload: [ ";
    for (auto b : payload) {
...
void Wheel_Controller_test::runTestRoutine() {
...
int main() {
    Wheel_Controller_test test("/dev/can0");
